# ============================================================================
# RESIZES TO 2048x2048 PIXELS THE ORIGINAL MHIs
# ============================================================================
#
...
# │   │           └── mhi_overlay.png        # Final overlay output (CREATED BY THIS SCRIPT)
# │   └── subfolder_2/
# │       └── shortened_Y/
# │           └── subsubfolder_2/
# │               ├── npy_tracks/
...
# │               │   └── aligned_blobs_*.npy
# │               └── mhi_overlay.png        # OUTPUT: Created by this script
...
#    - Finds all .npy files in alignment_results/ folder
#    - Loads blob coordinates from each .npy file
#    - Draws colored circles on the MHI image (different color per file)
#    - Saves the final overlay as mhi_overlay.png
#
# Input Files:
# - *_mhi.png: Background Motion History Image (must exist in each subsubfolder)
# - alignment_results/*.npy: Blob coordinate files with format [x, y, timepoint, ...]
#
# Output Files:
# - mhi_overlay.png: Final visualization with all blobs overlaid (created in each subsubfolder)
#
...
def load_blob_data(npy_file):
    ...
    try:
        ...
        print(f"  Loaded {len(data)} blobs from {Path(npy_file).name}")
        return data
...
def draw_all_blobs_on_image(image_path, all_blob_coordinates, output_path, blob_color, blob_size):
    ...
    try:
        ...
        print(f"  Drew {blob_count} blobs on image (resized to 2048x2048)")
        return True
...
def process_single_subsubfolder(subsubfolder_path, color_list, blob_size):
    ...
    # Define paths
    alignment_results_path = os.path.join(subsubfolder_path, "alignment_results")

    # Check if alignment_results folder exists
    if not os.path.exists(alignment_results_path):
        print(f"  Warning: alignment_results folder not found")
        return False, "alignment_results folder not found"
    ...
    # Get all npy files from alignment_results
    npy_files = glob.glob(os.path.join(alignment_results_path, "*.npy"))
    print(f"  Found {len(npy_files)} .npy files in alignment_results")

    if not npy_files:
        print("  No .npy files found in alignment_results!")
        return False, "No .npy files found in alignment_results"

    # Set output path
    output_path = os.path.join(subsubfolder_path, "mhi_overlay.png")
    ...
    try:
        ...
        for npy_file in sorted(npy_files):  # Sort for consistent processing order
            ...
            print(f"    Found {blob_count} total blobs")
        ...
        print(f"  SUCCESS: Processed {total_processed} files with {total_blobs} total blobs")
        print(f"  Output saved to: {output_path}")
        return True, f"Processed {total_processed} files, {total_blobs} blobs"
...
def batch_process_blob_visualization(segmentation_output_path, color_list, blob_size):
    """
    Main function to batch process all subsubfolders for blob visualization.
    ...
    print(f"Starting batch processing of: {segmentation_output_path}")
    print("=" * 80)
