# ==============================================================================
# Data Processing Script for Blob Tracking Coordinate Conversion
# ==============================================================================
#
# Description:
...
#   * alignment_results/ folder with .npy tracking data files
#
# Output:
...
# │   │           ├── alignment_summary.txt  # Summary of alignment results
# │   │           ├── alignment_results/     # INPUT: Contains .npy files to process
# │   │           │   └── aligned_blobs_*.npy # Source tracking data files
# │   │           ├── alignment_converted_results/ # OUTPUT: Created by this script
# │   │           │   └── *_converted.npy    # Converted tracking data (micrometers & seconds)
# │   │           ├── mhi_overlay.png        # Final overlay output
...
# │               ├── alignment_results/     # INPUT: Source .npy files
# │               ├── alignment_converted_results/ # OUTPUT: Converted .npy files
# │               ├── mhi_overlay.png
...
def process_npy_files(root_directory, pixel_size_um, processed_resolution, camera_resolution):
    """
    Process .npy files in the segmentation_output folder structure
    ...
    summary = {
        ...
        'empty_files': [],
        'folders_without_alignment_results': [],
    ...
    for subfolder in segmentation_output_path.iterdir():
        ...
        for shortened_folder in subfolder.iterdir():
            ...
            for changed_folder in shortened_folder.iterdir():
                ...
                # Look for alignment_results folder
                alignment_results_path = changed_folder / "alignment_results"
                if not alignment_results_path.exists():
                    print(f"    No alignment_results folder found in {changed_folder.name}. Skipping.")
                    summary['folders_without_alignment_results'].append(str(changed_folder))
                    continue

                # Create output directory for converted results
                output_dir = changed_folder / "alignment_converted_results"
                output_dir.mkdir(exist_ok=True)
                print(f"    Output directory: {output_dir.name}")

                # Get a list of all .npy files in the alignment_results folder
                ...
                for npy_file in npy_files:
                    print(f"      Processing file: {npy_file.name}")
                    ...
                    try:
                        ...
                        converted_data[:, 2] = data[:, 2] * time_scale  # Scale time points

                        # Save the converted data
                        np.save(converted_file_path, converted_data)
                        print(f"      Converted data saved to {converted_file_path.name}")
...
def print_processing_summary(summary):
    ...
    print(f"   - Files successfully converted: {summary['total_files_processed']}")
    ...
    if summary['successful_conversions']:
        ...
        for conversion in summary['successful_conversions']:
            ...
            print(f"     Data points: {conversion['data_points']}, Frequency: {conversion['frequency_used']:.2f} Hz")
    ...
    # Folders without alignment results
    if summary['folders_without_alignment_results']:
        print(f"\nFOLDERS WITHOUT ALIGNMENT_RESULTS ({len(summary['folders_without_alignment_results'])} folders):")
        for folder in summary['folders_without_alignment_results']:
    ...
    if summary['total_files_processed'] > 0 and len(summary['failed_conversions']) == 0:
        ...
        print("PROCESSING COMPLETED WITH SOME ISSUES")
    else:
...
if __name__ == "__main__":
    ...
    processing_summary = process_npy_files(ROOT_DIRECTORY, PIXEL_SIZE_UM, PROCESSED_RESOLUTION, CAMERA_RESOLUTION)
