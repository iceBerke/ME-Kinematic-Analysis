# MHI-Track Blob Alignment Processor with Sequential Filtering
#
...
# │   │           └── alignment_results/     # Alignment output (CREATED BY THIS SCRIPT)
...
def process_subsubfolder(root_path, subfolder, shortened_folder, subsubfolder, 
    ...
    try:
        ...
        # Output: alignment results directory
        alignment_results_dir = segmentation_path / "alignment_results"
        ...
        # Process the dataset
        summary_file = segmentation_path / "alignment_summary.txt"
...
def process_dataset(mhi_file, blobs_input_file, tracks_folder, alignment_results_dir, 
    ...
    for track_path in track_files:
        ...
        try:
            ...
            # STEP 1: Find aligned blobs (all blobs that overlap with track)
            blobs_on_track_segment = get_blobs_on_segment(blobs, track_segment, mhi, MHI_TIME_TOLERANCE)
            ...
            if len(blobs_on_track_segment) > 0:
                ...
                if APPLY_TEMPORAL_FILTERING:
                    ...
                    print(f"      Track alignment: {len(aligned_blobs)} blobs found")
                    ...
                    print(f"      Found {len(filtered_blobs)} aligned blobs (no temporal filtering)")
                ...
                if len(filtered_blobs) > 0:
                    ...
                    print(f"      Final result: {len(sorted_blobs)} blobs saved to {output_filename}")
                    ...
                    if APPLY_TEMPORAL_FILTERING and blobs_removed > 0:
                        track_summary.append(f"{track_path.name}: {len(sorted_blobs)} aligned blobs (removed {blobs_removed} temporal duplicates)")
                    else:
                        track_summary.append(f"{track_path.name}: {len(sorted_blobs)} aligned blobs")
                else:
    ...
    print(f"      Total final aligned blobs: {track_stats['total_aligned_blobs']}")
...
def save_alignment_summary(summary_file, track_summary, total_aligned_blobs, mhi_file, 
    ...
    try:
        with open(summary_file, 'w') as f:
            f.write("MHI-Track Blob Alignment Summary\n")
            f.write("=" * 50 + "\n\n")
            ...
            f.write("\n")
            f.write("-" * 30 + "\n")
            f.write(f"Total final aligned blobs: {total_aligned_blobs}\n")
...
def get_blobs_on_segment(blobs, track_segment, mhi, MHI_TIME_TOLERANCE):
    """Find blobs that fall on a track segment within time tolerance."""
    ...
    for blob in blobs:
        ...
        if (0 <= blob_y < track_segment.shape[0] and 
            ...
            track_segment[blob_y, blob_x] > 0 and 
            in_time_tolerance_range(blob, mhi, MHI_TIME_TOLERANCE)):
