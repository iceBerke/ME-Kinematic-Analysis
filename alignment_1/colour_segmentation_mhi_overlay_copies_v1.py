# ============================================================================
# Copy MHI Overlays Script
# ============================================================================
#
# This script collects all mhi_overlay.png files created by the batch mhi
...
# Output Structure:
# root_directory/
# └── mhi_overlays/                          # Created by this script
#     ├── Lead_1_sh5_subsubfolder_1.png            # Renamed copy
#     └── Lead_2_sh10_subsubfolder_2.png           # Renamed copy
#
# Script Workflow:
# 1. Scans segmentation_output/ for all mhi_overlay.png files
# 2. Extracts subfolder name, shortened folder number and subsubfolder name
# 3. Creates mhi_overlays/ directory if it doesn't exist
# 4. Copies files with new naming convention: {subfolder_name}_sh{X}_{subsubfolder_name}.png
...
def find_all_mhi_overlays(segmentation_output_path):
    """Find all mhi_overlay.png files and extract path information with memory efficiency"""
    overlay_files = []
                ...
                for shortened_path in shortened_folders:
                    ...
                    try:
                        ...
                        for subsubfolder_path in subsubfolders:
                            # Check for mhi_overlay.png
                            overlay_file = subsubfolder_path / "mhi_overlay.png"
                            if overlay_file.exists() and overlay_file.is_file():
                                overlay_files.append({
                                    ...
                                    'new_name': f"{subfolder_path.name}_sh{shortened_num}_{subsubfolder_path.name}.png"
                                })
...
def copy_mhi_overlays(root_directory_path):
    print(f"Starting MHI overlay collection from: {root_directory_path}")
    print("=" * 80)

    # Define paths using pathlib
    root_path = Path(root_directory_path)
    segmentation_output_path = root_path / "segmentation_output"
    mhi_overlays_dir = root_path / "mhi_overlays"
    ...
    # Find all mhi_overlay.png files
    overlay_files = find_all_mhi_overlays(segmentation_output_path)

    print(f"Found {len(overlay_files)} mhi_overlay.png files")

    if not overlay_files:
        print("No mhi_overlay.png files found!")
        return {"total": 0, "successful": 0, "failed": 0, "success_rate": 0.0}

    # Create mhi_overlays directory if it doesn't exist
    ...
    print("\n" + "=" * 80)
    print("MHI OVERLAY COPYING COMPLETE")
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    results = copy_mhi_overlays(root_directory_path)

    # Optional: Print final summary
    if results["total"] > 0:
        print(f"\nFinal Summary:")
        print(f"  {results['successful']}/{results['total']} files copied successfully")
