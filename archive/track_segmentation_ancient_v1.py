...
def process_image_for_tracks(image_path, colors, output_folder):
    ...
    # Create the subfolder named after the image within the output folder
    image_base_name = os.path.splitext(os.path.basename(image_path))[0]
    subfolder_path = os.path.join(output_folder, f"{image_base_name}")
    ...
    for color in colors:
        ...
        if non_zero_count > 0:
            ...
            # Save the resulting image in the subfolder with sequential naming
            track_image_path = os.path.join(subfolder_path, f"track_{track_counter}.png")
...
def process_all_images(parent_directory, colors):
    """
    Process all TIFF images in the parent directory and save the results in subfolders.
    """
    # Create the 'analysed_tracks' subfolder in the parent directory
    output_folder = os.path.join(parent_directory, "analysed_tracks")
...
# Example usage
parent_directory = "/home/berke-dhm/Desktop/MHI_colour_test/coloured_MHIs"  # Path to the parent directory
colors = [
    (254, 209, 209),
    (226, 209, 209),
    (211, 209, 209),
