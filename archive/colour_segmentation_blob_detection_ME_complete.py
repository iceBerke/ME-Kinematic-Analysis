...
# Remove unused functions for memory efficiency
# def solidity(contour): - moved inline
# def extract_blob_contour(): - replaced with efficient version
...
def calculate_blob_properties_efficient(img_clahe, kp):
    ...
    if x2 - x1 == 0 or y2 - y1 == 0:
        # Return single pixel value for brightness, 0 for solidity
        return float(img_clahe[y, x]), 0.0

    patch = img_clahe[y1:y2, x1:x2].copy()  # Small local copy only

    # Calculate baseline brightness from patch (always possible)
    brightness = float(np.mean(patch))

    # Attempt to calculate solidity
    solidity_value = 0.0

    # Use simple thresholding on the patch
    _, binary = cv.threshold(patch, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU)

    # Find contours in the small patch
    contours, _ = cv.findContours(binary, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)

    # If contours found, calculate solidity and try to improve brightness
    if len(contours) > 0:
        # Find the largest contour (most likely to represent the blob)
        best_contour = max(contours, key=cv.contourArea)

        # Calculate solidity - even for very small areas
        ...
        # Try to calculate more accurate brightness using the contour mask
        # Note: We use contour directly without shifting since it's already in patch coordinates
        try:
            ...
            # If mask calculation fails, keep patch mean brightness
            pass
...
def detect_blobs_single_image(image_path,
    ...
    # Process blobs immediately and extract data to avoid holding large image in memory
    blob_data_list = []
    for kp in filtered_keypoints:
        # Calculate brightness and solidity immediately
        brightness, sol = calculate_blob_properties_efficient(img_clahe, kp)
        # Calculate area using same formula as sequential filtering script
    ...
    del img, img_resized, img_clahe, img_inverted
        ...
        for shortened_folder in shortened_folders:
            ...
            for subsubfolder in shortened_path.iterdir():
                ...
                for image_file in sorted(image_files):  # Sort to ensure consistent ordering
                    ...
                    # Detect blobs and get processed data using current subfolder parameters
                    blob_data_list = detect_blobs_single_image(str(image_path), **current_params)

                    if blob_data_list is not None:
                        # Add timepoint to each blob and collect: [x, y, t, area, brightness, solidity]
                        for blob_data in blob_data_list:
                            # Insert timepoint at index 2: [x, y, t, area, brightness, solidity]
                            complete_blob_data = [blob_data[0], blob_data[1], timepoint, blob_data[2], blob_data[3],
                                                  blob_data[4]]
                            all_blob_data.append(complete_blob_data)
                ...
                if all_blob_data:
                    ...
                    print(f"      Array shape: {blob_array.shape} (columns: x, y, t, area, brightness, solidity)")
                else:
                    print(f"      No blobs detected in {subsubfolder.name}")


def load_and_inspect_blob_data(npy_file_path):
    """Load and display summary information about enhanced blob detection data"""
    data = np.load(npy_file_path)
    print(f"Blob data shape: {data.shape}")
    print("Columns: x, y, t (timepoint), area, brightness, solidity")
    print(f"Number of blobs: {data.shape[0]}")
    if data.shape[0] > 0:
        print(f"Timepoint range: {data[:, 2].min()} to {data[:, 2].max()}")
        print(f"Area range: {data[:, 3].min():.2f} to {data[:, 3].max():.2f} pixels²")
        print(f"Brightness range: {data[:, 4].min():.2f} to {data[:, 4].max():.2f}")
        print(f"Solidity range: {data[:, 5].min():.3f} to {data[:, 5].max():.3f}")
        print("\nFirst few blobs:")
        print(data[:5])
    return data
...
if __name__ == "__main__":
    ...
    subfolder_params = {
        ...
        'Copper_3': {
            ...
            'dark_minThreshold': 0,
            'dark_maxThreshold': 125,
            'dark_minArea': 15,  # Smaller minimum area for dark blobs
            'dark_maxArea': 800,
            'bright_minThreshold': 70,  # Lower threshold for bright blobs
            'bright_maxThreshold': 255,
            'bright_minArea': 25,  # Smaller minimum area for bright blobs
            'bright_maxArea': 800
        },
        'Copper_4': {
            'min_distance': 30,
        ...
        'Nickel_1': {
            ...
            'dark_minArea': 25,
            'dark_maxArea': 800,
            'bright_minThreshold': 65,  # Lower threshold for bright blobs
            'bright_maxThreshold': 255,
            'bright_minArea': 25,
            'bright_maxArea': 800
        },
        ...
        'Nickel_4': {
            ...
            'dark_minArea': 25,
            'dark_maxArea': 800,
            'bright_minThreshold': 85,  # Slightly lower than default
            'bright_maxThreshold': 255,
            'bright_minArea': 25,
            'bright_maxArea': 800
        }
    ...
    print(f"Starting batch blob detection with enhanced parameters...")
    ...
    print(f"Output format: [x, y, t, area, brightness, solidity]")
    ...
    print("NPY files now contain 6 columns: [x, y, timepoint, area, brightness, solidity]")
