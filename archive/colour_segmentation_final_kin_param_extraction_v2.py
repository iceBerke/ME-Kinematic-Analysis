...
# │   │           ├── alignment_converted_results/ # INPUT: Contains .npy files to process
...
# │               ├── alignment_results/     # Contains original .npy files (not used)
# │               ├── alignment_converted_results/ # INPUT: Source .npy files for processing
# │               ├── mhi_overlay.png
# │               └── average_frequency.txt
# ├── processed_results/                     # OUTPUT: Created by this script
# │   ├── Lead_1_processed_results.csv   # Analysis results for Lead_1
# │   └── Copper_3_processed_results.csv # Analysis results for Copper_3
...
# 2. For each directory, find all alignment_converted_results/ directories recursively
# 3. Process all *_converted.npy files (containing x, y, t coordinates)
# 4. Calculate motion metrics with smoothing and filtering
# 5. Save individual track results as CSV files in processed_results/ directory
...
#
# Input: *_converted.npy files with columns [x_um, y_um, time_s]
...
processed_results_dir = root_data_dir / "processed_results"
...
def extract_sample_group(subsubfolder_name):
    ...
    # Pattern to match everything up to _sampleX_recY
...
def write_summary_file(output_file, grouped_data, subfolder_name):
    ...
    with open(output_file, 'w') as f:
        f.write(f"SUMMARY STATISTICS FOR {subfolder_name}\n")
        f.write("="*60 + "\n\n")
...
for subfolder in segmentation_output_dir.iterdir():
    if subfolder.is_dir():
        ...
        # Find all alignment_converted_results directories within this subfolder
        alignment_converted_paths = list(subfolder.rglob("alignment_converted_results"))
        
        if not alignment_converted_paths:
            print(f"  WARNING: No alignment_converted_results directories found in {subfolder.name}")
        ...
        with open(output_file, mode='w') as f:
            ...
            # Process each alignment_converted_results directory
            for alignment_dir in alignment_converted_paths:
                subfolder_sessions += 1
                print(f"  Processing session: {alignment_dir.parent.name}")
                
                # Get the session identifier (subsubfolder_()_changed)
                session_name = alignment_dir.parent.name
                
                # Find all .npy files in alignment_converted_results (looking for converted files)
                ...
                # Process each npy file in the current alignment_converted_results folder
...
print("Track metrics and direction change timepoints saved as CSV files in processed_results folder.")
