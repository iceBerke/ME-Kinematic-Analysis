# Individual track segmentation of colored MHIs, with the following file extension: "_mhi_color.tif" or "_mhi_colors.tif"
# The code should receive a global root directory with different experiments (e.g., Lead_1, Lead_2, etc.)
# Inside each experiment folder, we should have our original recordings and the shortened folders
# It will then make sure to copy the original MHIs and the segmented track images (from each validate folder,
# within the different recordings folders in the shortened directory) into an output directory inside the root
# The output will mirror the organization of the input: root>output_folder>experiment_directory>shortened_directory>recording_directory
# It will also only analyze the folders that have coloured MHIs!
...
def find_mhi_color_images(root_directory):
    """
    Find all *mhi*_color.tif and *mhi_colors.tif images in validate folders and return their paths with metadata.
    """
    ...
    for experiment_dir in root_path.iterdir():
        ...
        for item in experiment_dir.rglob("shortened_*"):
            ...
            for rec_folder in item.iterdir():
                ...
                # Find MHI color files - now supporting both patterns
                mhi_color_files = []
                # Pattern 1: *mhi*_color.tif
                mhi_color_files.extend(list(validate_folder.glob("*mhi*_color.tif")))
                # Pattern 2: *mhi_colors.tif
                mhi_color_files.extend(list(validate_folder.glob("*mhi*_colors.tif")))

                # Find corresponding PNG files
                mhi_png_files = list(validate_folder.glob("*mhi*.png"))

                if mhi_color_files:
                    for mhi_color_file in mhi_color_files:
                        # Try to find a corresponding PNG file with similar name
                        mhi_png_file = None
                        for png_file in mhi_png_files:
                            # Check if PNG file has similar base name (without _color/_colors suffix)
                            color_base = mhi_color_file.stem.replace('_color', '').replace('_colors', '')
                            png_base = png_file.stem
                            if color_base == png_base or png_base in color_base or color_base in png_base:
                                mhi_png_file = png_file
                                break

                        # If no specific match found, use the first PNG file
                        if not mhi_png_file and mhi_png_files:
                            mhi_png_file = mhi_png_files[0]
                    ...
                    print(f"      No *mhi*_color.tif or *mhi_colors.tif file found in {validate_folder}")

    return mhi_images


def process_all_mhi_images(root_directory, colors, min_area=10):
    """
...
# Run the processing
process_all_mhi_images(root_directory, colors, min_area=20)
